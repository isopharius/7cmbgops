...
call seven_fnc_fillStations;

_volume ctrlAddEventHandler['SliderPosChanged', {
	[round (_this select 1)] call seven_fnc_volume;
}];
...
_stopButton ctrlAddEventHandler['ButtonClick', seven_fnc_stop];

_addButton = _display displayCtrl 13408;
_addButton ctrlAddEventHandler['ButtonClick', seven_fnc_startAdd];
...
_removeButton ctrlAddEventHandler['ButtonClick', {
	...
	if (_selectedStationIndex != -1) then {
		...
		_result = [_name] call seven_fnc_removeFromProfile;
		if (_result) then {
