...
if ((NOT dtaHaveAimpoint) AND (NOT _prePlotted)) exitWith {nul = [_prePlotted] execVM "\7cmbgops\scripts\Artillery\Dialog\Aimpoint.sqf"};
...
//((findDisplay _iccCode) displayCtrl (_iccCode + 1)) ctrlSetText format ["%1: %2 x%3  GRID:%4-%5",_assetName,_name,(count _tubes),_xText,_yText];
...
//systemChat format [">> %1 %2",dtaSelectedTubeIndex,_tubes];
dtaSelectedTube = _tubes select dtaSelectedTubeIndex;
//systemChat format ["> %1",dtaSelectedTube];
nul = [] execVM "\7cmbgops\scripts\Artillery\Dialog\DisplayWarheads.sqf";
