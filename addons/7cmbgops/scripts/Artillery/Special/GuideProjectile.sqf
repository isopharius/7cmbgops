...
//_perSecondChecks = 100; //direction checks per second
...
_secondaryTarget = createVehicle ["HeliHEmpty",_defaultTargetPos, [], 0, "CAN_COLLIDE"];
...
//		_dispersion = (_missile distance _defaultTargetPos) / 1;
		_dispersionMin = _dispersion / 10;
//		_dispersionMin = _dispersion / 1;
	...
	//_defaultTargetPos = [(_defaultTargetPos select 0),(_defaultTargetPos select 1),((_defaultTargetPos select 2) + 100)];
	//systemChat format ["%1 %2",_defaultTargetPos,_target];
	...
	if (_missile distance _target > (_missileSpeed / 20)) then {
		...
		//_relDirVer = asin ((((getPosASL _missile) select 2) - (((getPosASL _target) select 2) + 2.5)) / (_target distance _missile));
