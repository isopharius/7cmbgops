...
_trackEH = 0;
if (dtaTrackRounds) then {_trackEH = _tube addEventHandler ["fired",{_this execVM "\7cmbgops\scripts\Artillery\Test\Fired.sqf"}]};
...
if (_warheadType in dtaLaserTypes) then {
	_fuse = "";
	_action = 1;
	_specialEH = _tube addEventHandler ["fired", compile format ["[_this,%1,%2] execVM '\7cmbgops\scripts\Artillery\Special\Fired.sqf'",_action,_pos]];
};
...
if (_fuse isEqualTo "AIRBURST") then {
	if (_airburstHeight isEqualTo "LOW") then {_airburstEH = _tube addEventHandler ["fired",{_this execVM "\7cmbgops\scripts\Artillery\Airburst\AirburstFireLow.sqf"}]};
	if (_airburstHeight isEqualTo "MED") then {_airburstEH = _tube addEventHandler ["fired",{_this execVM "\7cmbgops\scripts\Artillery\Airburst\AirburstFireMed.sqf"}]};
	if (_airburstHeight isEqualTo "HIGH") then {_airburstEH = _tube addEventHandler ["fired",{_this execVM "\7cmbgops\scripts\Artillery\Airburst\AirburstFireHigh.sqf"}]};
};
...
while {(_rounds > 0)} do {
	...
	// Aim tube
	_gunner doWatch [_posx,_y,_z];

	if (_firstRound) then {
		if (_gunAngle isEqualTo "LOW") then {sleep 5};
		if (_gunAngle isEqualTo "HIGH") then {sleep 10};
	};
	...
	_tube fire [_tubeType,_charge];
	//sleep _sleepTime;

	//waitUntil {(_tube getVariable "FIRED") isEqualTo 1};
	...
	if (_firstRound) then {
		if (_tube isEqualTo _dtaSelectedTube) then {
			...
			[_ETA,_asset] execVM "\7cmbgops\scripts\Artillery\Control\Splash.sqf";
		};
	};
	_firstRound = false;
	sleep _sleepTime;
