...
//systemChat format ["RT: %1",(typeOf _round)];

_time = 0;
_time = time;
_markerName = format ["%1%2%3",_round,_time,(random 999)];
_marker = createMarker [_markerName,(getPos _round)];
...
while {(alive _round)} do {
	if (alive _round) then {_marker setMarkerPos (getPos _round)};
	sleep 0.1;
	if (dtaDebug) then {_round execVM "\7cmbgops\scripts\Artillery\Test\Track2.sqf"};
};
