...
private["_groupName","_1","_2","_3","_out","_array1","_array2","_numArray","_count","_x","_tempName","_text"];
...
while {((count _array1) > 0)} do
{
	...
	while {((count _array2) > 0)} do
	{
		...
		if (_groupName isEqualTo _tempName) then {_out = format ["%1 %2",_2,_3]};
		if (_groupName isEqualTo _tempName) then {_array1 = []};
		if (_groupName isEqualTo _tempName) then {_array2 = []};
	};
};
_out