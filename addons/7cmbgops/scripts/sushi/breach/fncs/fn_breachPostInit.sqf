...
[] spawn {
	...
	Diag_Log format ["Breach PostInit has been executed"];

	//-->Handle gun breach
	if (SAF_MISSION_SET(breach_allowGunBreach) > 0 && !isDedicated) then {
		//-->Add event handler
		player addEventHandler ["FIRED",seven_fnc_breachFiredEh];
	};
	...
	call seven_fnc_lockpickcheck;
	player addeventhandler ["respawn","call seven_fnc_lockpickcheck;"];
	};
	};

	if (SAF_MISSION_SET(breach_allowExplosiveBreach) > 0 && !isDedicated) then {  //Add explosive breaching
	if (!isNull Player) then {
	call seven_fnc_checkexplosive;
	player addeventhandler ["respawn","call seven_fnc_checkexplosive;"];
	};
	};
};
