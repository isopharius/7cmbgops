...

{
	...
	_cacheBuildings = [_cityPos,_cityRadA] call seven_fnc_findBuildings;

	for "_i" from 1 to (6 + (floor (random 4))) step 1 do {
		if(count _cacheBuildings > 0) then {
			...
			_intelPosition = [_targetBuilding] call seven_fnc_RandomBuildingPosition;
