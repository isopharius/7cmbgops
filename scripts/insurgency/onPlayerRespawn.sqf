...
if (paramsArray select 4 == 1) then {
	if ((_unitType in INS_AUTORIFLE) || (_unitType in INS_ENGINEERS)) then {
		...
		_id = player addaction ["<t color='#FFFF00'>Deploy Sandbag</t>", seven_fnc_deploySandbag, nil, -10, false];
	...
	if (_unitType in INS_MEDICS) then {
		...
		_id = player addaction ["<t color='#FF0000'>Deploy MASH</t>", seven_fnc_deployMedicTent, nil, -10, false];
