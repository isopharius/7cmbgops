...
switch (toLower (_locality)) do {
	...
	case "client": {
		...
		[[[player,_position,_radius],{
			["server",[(_this select 0),(_this select 1),(_this select 2)]] call STC_fnc_requestRecon;
	...
	case "server": {
		...
		_profileArray = [str side _player] call STC_fnc_getSideInfantry;
		...
		for "_x" from 1 to (count _profileArray - 1) step 1 do {
			...
			if !([_profile,"isPlayer",false] call ALIVE_fnc_hashGet) then {
				...
				["task",[_profile,_position,_player]] spawn STC_fnc_requestRecon;
				_loops = _loops + 1;
				sleep .3;
			};
		};
	...
	case "task": {
		...
		_enemySides = [([_profile,"side"] call ALIVE_fnc_hashGet)] call STC_fnc_getEnemySides;
		_enemies = [];
		{
		...
		//-- Create markers
		{
			...
			[_marker] spawn STC_fnc_markerFade;
